...
const G4String SN_OpticalSimMaterials::path = "/nemo243/Optical_Simulation_Arnaud/simulation_input_files/";
...
void SN_OpticalSimMaterials::Construct()
{
  ...
  //Teflon
  teflon = new G4Material("teflon", 0.4*g/cm3, 2);
  ...
  polysty = new G4Material("polysty",  
			   1.053*g/cm3, //1.053 pour valeur moyenne mesurée à Bordeaux
			   2, 
			   kStateSolid, 
			   273.15*kelvin, 
			   1.0*atmosphere );
  ...
  if (Readabsorb.is_open()){
    while(!Readabsorb.eof()){
      ...
      Absorb[absorbEntries] = 1.3*varabsorblength*m; 
  ...
  if(Read_ref_index.is_open()){
    while(!Read_ref_index.eof()){
      ...
      ref_index_value[ref_index_Entries] =1.00 * ref_index_value[ref_index_Entries];
  ...
  scintMPT->AddConstProperty("WLSTIMECONSTANT",1.5*ns);
  ...
  //scintillator->GetIonisation()->SetBirksConstant(0.126*mm/MeV); // 1ère valeur pour simu optique prise en considération
  scintillator->GetIonisation()->SetBirksConstant(0.22*mm/MeV); // 2ère valeur pour simu optique prise en considération
 ...
 G4int PMMA_ref_index_Entries = 0;
  ...
  // pmmabulkAbsorb[pmmaEntries] = 3.0*m;
