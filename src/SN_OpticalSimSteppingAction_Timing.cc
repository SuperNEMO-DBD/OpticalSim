...
const G4String SN_OpticalSimSteppingAction::path = "/nemo243/Optical_Simulation_Arnaud/simulation_input_files/";
  ...
  if(particleType == G4OpticalPhoton::OpticalPhotonDefinition()){
      ...
      case Detection: 
	{
	  ...
	  if (fnum < Uniformity)
	    {
	      evtac->CountDetected();
	      // G4cout << "Local Time = " << aStep->GetPreStepPoint()->GetLocalTime()/ns << " ns " << G4endl;
	      // G4cout << "Global Time = " << aStep->GetPreStepPoint()->GetGlobalTime()/ns << " ns " << G4endl;
	      // G4cout << "Proper Time = " << aStep->GetPreStepPoint()->GetProperTime()/ns << " ns " << G4endl;
	      ...
	      double TTS = 2.251; //Mean Value TTS
	      ...
	      */
	      ...
	      TF1 *pTP_distribution = new TF1("pTP_distribution","(1/(5))*(exp(-x/5))",0,70);
	      TF1 *POPOP_distribution = new TF1("POPOP_distribution","(1/(1.5))*(exp(-x/1.5))",0,70);
			...
			Statisticster.G4TimeSc = aStep->GetPostStepPoint()->GetGlobalTime()/ns;
			Statisticster.G4TimeTotal = Statisticster.G4TimeSc + Time_Transit_PMT;


			Statisticster.Reflections = 	  ((SN_OpticalSimTrackInformation*)(aStep->GetTrack()->GetUserInformation()))->GetTotalInternalReflections();
			// G4cout << "Time Emission pTP [calculated] = " << Statisticster.TimeEmissionScPTP << " ns " << G4endl;
			// G4cout << "Time Total Sc [calculated] = " << Statisticster.TimeTotalSc << " ns " << G4endl;
			// G4cout << "Time Total Sc [G4] = " << Statisticster.G4TimeSc << " ns " << G4endl;
			// G4cout << "Time Total PMT [all] = " << Statisticster.TimeTransitPMT << " ns " << G4endl;
			// G4cout << "Time Total [calculated] = " << Statisticster.TimeTotal << " ns " << G4endl;
			// G4cout << "Time Total [G4] = " << Statisticster.G4TimeTotal << " ns " << G4endl;
      ...
      case SpikeReflection:
	{
	  ...
	  //G4cout << "Reflection " << ((SN_OpticalSimTrackInformation*)(aStep->GetTrack()->GetUserInformation()))->GetReflections() <<  G4endl;
  ...
  */
  ...
  if ((aStep->GetTrack()->GetParentID() == 0)
      ...
      && (aStep->GetPostStepPoint()->GetPhysicalVolume()->GetName() == "PhysicalScint"))
    {
      ...
      /*
      if(partname == "gamma" && endproc == "compt")
	{
  ...
  if(partname == "opticalphoton" && aStep->GetTrack()->GetCreatorProcess()->GetProcessName() == "Scintillation" && StepNo ==1)
    {
      ...
      // G4cout << "Global Time pre step = " << aStep->GetPreStepPoint()->GetGlobalTime()/ns << " ns " << G4endl;
      // G4cout << "Global Time post step = " << aStep->GetPostStepPoint()->GetGlobalTime()/ns << " ns " << G4endl;
      //Statisticster.G4TimeSc = aStep->GetPostStepPoint()->GetGlobalTime()/ns;
      // runac->UpdateStatisticster(Statisticster);
    }
