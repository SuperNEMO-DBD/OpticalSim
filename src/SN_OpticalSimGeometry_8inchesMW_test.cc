...
const G4String SN_OpticalSimGeometry::path_bin = "/nemo243/Optical_Simulation_Arnaud/bin/";
const G4String SN_OpticalSimGeometry::path = "/nemo243/Optical_Simulation_Arnaud/simulation_input_files/";
...
G4VPhysicalVolume* SN_OpticalSimGeometry::Construct( ){
  ...
  //LogicalSNMW_8InchesScint->SetUserLimits(fStepLimit);
  ...
  G4String PMTfile = path+"QE_Mean.txt"; 
...
//##########################################################################################
  ...
  if (ReadPMT.is_open()){
    while(!ReadPMT.eof()){
      ...
      //PMTdetect[PMTentries] = quant_eff*0.83;  //For the GAxxxx file with good QE and 0.65 for collection efficiency & 0.57 for CU
      PMTdetect[PMTentries] = quant_eff*0.75;  //From the model 8inches !!!
